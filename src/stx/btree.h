	...
	*/
	...
	private:
				...
				if(super::ptr!=NULL_REF){
					if(super::ptr->refs == 0 && super::ptr->is_orphaned()){					
						if(has_context()){
							...
							printf("[ERROR] [TS] context does not exist\n");
						}
						ptr = NULL_REF;
					}
				}
		...
		public:
			...
			inline int find_lower(key_compare key_less,key_interpolator interp, const key_type* keys, const key_type& key) const {
				...
				if(false && cacheslotmax && get_cache_occupants()){
					...
					int m;
					while(l < h) { //		() {  h-l > traits::max_scan (h-l > traits::max_scan) { //
						m = (l + h) >> 1;
		...
		public:
			...
			/// accessor
			inline key_type *keys(){
				return &_keys[0];
			}
			inline const key_type *keys() const {
				return &_keys[0];
			}
			inline data_type *values(){
				return &_values[0];
			}
			inline const data_type *values() const{
				return &_values[0];
			}
			...
			void load(btree* context,
				...
				if(d != bsize){
					...
					printf("[ERROR] [TS] surface nodes of invalid size\n");
				}
		...
		void save(surface_node* n, stream_address& w){
			...
			if(n->is_modified()){
				...
				if(n->get_address() > 0 && n->get_address() == n->get_next().get_where()){
					printf("[TS] [ERROR] saving node with recursive address\n");
				}
	...
	public:
		...
		public:
			...
			{
				//return _keys[current_slot];
				key_type& test = currnode->keys()[current_slot];
				return test;
			}
	...
	public:
		...
		void free_interiors(){
			for(auto i = interiors.begin(); i != interiors.end(); ++i){
				///new ((*i)) interior_node();
				allocation_pool.free_ns<interior_node>((*i));
			}
			interiors.clear();
		}
			...
			key_type* splitkey, typename node::ptr& splitnode)
		{
			...
			else // n->issurfacenode() == true
			{
				...
				{	/// this code only runs during fill
					...
					surface->set_slot_loader(source, child_at);
