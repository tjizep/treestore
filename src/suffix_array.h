...
public:
	...
	void encode(const char * buffer, nst::u64 length){
		...
		std::string storage_name = "suffix_temp.dat";
		{
			...
			_SuffixArray suffix_array(storage);
			typedef fe_hash_map<_SuffixArray> _HashFe;
			...
			size_t t = ::os::millis();
			suffix_hasher.set_backing(suffix_array);
			/// scan phase
			...
			suffix_hasher.flush();
			/// filtering away redundant suffixes
			_Inverted succinct;
			{
				_FreqPair prev_suffix;
				nst::u32 prev_max = 0;
				nst::u32 compared = 0;	
				_SuffixArray::iterator s = suffix_array.begin();
				prev_suffix.second = s.key();	
				
				++s;
				for(; s != suffix_array.end(); ++s){
					prev_suffix.first += s.data(); 	
					compared = prev_suffix.second.left_compare(s.key());
					if(compared >= prev_max){					
						prev_max = compared;
						prev_suffix.second = s.key();	
						prev_suffix.second.set_length(compared);
						
					}else{
						/// emit succinct suffixes
						succinct.push_back(prev_suffix);
						prev_suffix.second = s.key();	
						prev_max = 0; /// start over
					}
				}
			}
			/// reordering phase
			printf("There are %lld succinct suffixes\n",succinct.size());
			
			_Inverted frequencies;
			for(_SuffixArray::iterator s = suffix_array.begin(); s != suffix_array.end(); ++s){
				frequencies.push_back(std::make_pair(s.data(),s.key()));
			}
			...
			printf("created suffix array in %lld millis compressed to %lld bytes\n",::os::millis()-t, (size_t)(suffix_size*(noncoded+frequencies.size()))+(size_t)(writer - encoded.begin()));
			getch();
		}
