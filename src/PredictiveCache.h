...
namespace tree_stored{
	...
	struct _PredictiveCache{
		...
		_PredictiveCache():store_pos(0),hits(0),misses(0),loaded(false),multi(0),enabled(true){
		...
		bool load(){
			...
			if(!loaded){
				if(total_cache_size+sizeof(CachedRow)*CIRC_SIZE+sizeof(_Rid)*HASH_SIZE > MAX_PC_MEM){
					return false;
				}
